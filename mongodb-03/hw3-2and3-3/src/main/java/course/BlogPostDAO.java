...
public class BlogPostDAO {
	...
	public List<DBObject> findByDateDescending(int limit) {
		...
		// XXX HW 3.2, Work Here
		DBCursor cursor = postsCollection.find(new BasicDBObject());
	...
	public void addPostComment(final String name, final String email,
		...
		BasicDBObject comment = new BasicDBObject("author", name).append("body",
				body);
		...
		postsCollection.findAndModify(
				new BasicDBObject("permalink", permalink), new BasicDBObject(
						"$push", new BasicDBObject("comments", comment)));
